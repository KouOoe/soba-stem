...
def save_scan_position_diff(
    scan_position_init, scan_position_optim, save_dir, figsize=(8, 6)
):
    """
    ...
    # Draw vectors starting at (pos_x_flat, pos_y_flat) with displacement
    # components (dif_x_flat, dif_y_flat).
    plt.quiver(
        scan_position_init_x,  # X coordinates of each point
        scan_position_init_y,  # Y coordinates of each point
        diff_scan_pos_x,  # X displacement (difference)
        diff_scan_pos_y,  # Y displacement (difference)
        angles="xy",
        scale_units="xy",
        scale=1.0,
        color="red",
...
def save_cbed_diff(cbed_1, cbed_2, save_dir, probe_pos=(0, 0), figsize=(12, 3)):
    ...
    plt.savefig(
        save_dir + "/CBED_comparison_{:}_{:}.png".format(probe_pos[0], probe_pos[1]),
        bbox_inches="tight",
    )
    plt.close()
...
def potential_circle_plot(
    smatrix_np, optimised_Ug, reference_Ug, save_dir, figsize=(5, 4)
):

    smatrix_np.Ug_circle_plot(
        optimised_Ug,
        reference_Ug,
