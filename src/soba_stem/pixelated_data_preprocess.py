...
class PixelatedDataProcess:
    ...
    def D(self, ky, kx, inner, outer, theta1=-np.pi, theta2=np.pi, rot_offset=0):
        ...
        kx_rot = kx * np.cos(rot_offset + rot_epsilon) + ky * np.sin(
            rot_offset + rot_epsilon
        )
        ky_rot = -kx * np.sin(rot_offset + rot_epsilon) + ky * np.cos(
            rot_offset + rot_epsilon
        )

        mask_radius = np.logical_and(
            ky**2 + kx**2 < outer**2, ky**2 + kx**2 >= inner**2
        )
        mask_azimuth = np.logical_and(
            np.arctan2(ky_rot, kx_rot) >= theta1, np.arctan2(ky_rot, kx_rot) < theta2
        )
    ...
    def chi(self, ky, kx, defocus=0, tilt_rad=(0, 0)):
        chi = (
            ...
            * defocus
            * (
                self.wavelength * (ky**2 + kx**2)
                - 2 * ky * np.tan(tilt_rad[0])
                - 2 * kx * np.tan(tilt_rad[1])
            )
        )
        return chi

    def T(self, ky, kx, defocus=0, tilt_rad=(0, 0)):
        lens_transfer = self.A(ky, kx) * np.exp(
            -1j * self.chi(ky, kx, defocus, tilt_rad)
        )
    ...
    def cbed_import(self, cbed_data):
        ...
        self.kgrid = np.meshgrid(
            np.arange(
                -self.centroid[0], (np.shape(self.pacbed)[0] - self.centroid[0]), 1
            )
            * self.dky,
            np.arange(
                -self.centroid[1], (np.shape(self.pacbed)[1] - self.centroid[1]), 1
            )
            * self.dkx,
            indexing="ij",
        )
        ...
        self.qgrid = np.meshgrid(self.qy, self.qx, indexing="ij")
    ...
    def OBF_pixel_mask(self, qy, qx, thickness_input=None):
        ...
        kx_rot = self.kgrid[1] * np.cos(self.rot_offset) + self.kgrid[0] * np.sin(
            self.rot_offset
        )
        ky_rot = -self.kgrid[1] * np.sin(self.rot_offset) + self.kgrid[0] * np.cos(
            self.rot_offset
        )
        ...
        for islice in range(n_slice):
            ...
            mask += np.conj(self.T(ky_rot, kx_rot, z)) * self.T(
                ky_rot - qy, kx_rot - qx, z
            ) - self.T(ky_rot, kx_rot, z) * np.conj(self.T(ky_rot + qy, kx_rot + qx, z))
    ...
    def OBF_pixel(
        ...
        cbed = self.cbed_data
        intensity_BF = np.sum(
            np.sum(cbed * self.D(ky_rot, kx_rot, 0, 1, -np.pi, np.pi), axis=3), axis=2
        )
        ...
        g_func = (
            np.fft.fft2(self.cbed_data, axes=(0, 1))
            * self.ry
            * self.rx
            / (self.nry * self.nrx)
        )
        ...
        if sourcesize > 0:
            sourcesize_filter = self.source_blur_filter(
                source_fwhm=sourcesize, ratio_GtoL=ratio_GtoL
            )
        ...
        for iy in range(-self.nry // 2, self.nry // 2):
            for ix in range(-self.nrx // 2, self.nrx // 2):
                ...
                if qy**2 + qx**2 > 0 and qy**2 + qx**2 <= (2 * self.k0) ** 2:
                    ...
                    pctf[iy, ix] = (
                        (np.sum(mask * np.conj(mask) / (np.pi * self.k0**2))).real
                        * self.dky
                        * self.dkx
                    )
                    phase[iy, ix] = (
                        np.sum(g_func[iy, ix] * mask / (np.pi * self.k0**2))
                        * self.dky
                        * self.dkx
                    )
                ...
                progress = (
                    ((iy + self.nry // 2) * self.nrx + (ix + 1) + self.nrx // 2)
                    / (self.nry * self.nrx)
                    * 100
                )
        ...
        if output_mode == "real":
            ...
            if normalize_mode == "noise":
                ...
                obf = (
                    phase
                    * np.conj(noise)
                    / (np.conj(noise) * noise + np.max(np.abs(noise)) * 1e-2)
                    * scale
                )
                ...
                obf = (
                    phase
                    * np.conj(pctf)
                    / (np.conj(pctf) * pctf + np.max(np.abs(pctf)) * 1e-2)
                )
                obf = np.fft.ifft2(obf).imag / self.ry / self.rx * self.nry * self.nrx
    ...
    def COM(self, det_rot=0):
        ...
        COMx = np.sum(np.sum(self.cbed_data * kx_rot, axis=3), axis=2) / np.where(
            cbed_sum == 0, 1, cbed_sum
        )
        COMy = np.sum(np.sum(self.cbed_data * ky_rot, axis=3), axis=2) / np.where(
            cbed_sum == 0, 1, cbed_sum
        )
    ...
    def iCOM(self, det_rot=0):
        ...
        icom = (
            self.qgrid[1] * np.fft.fft2(COMx) + self.qgrid[0] * np.fft.fft2(COMy)
        ) / np.where(
            self.qgrid[0] ** 2 + self.qgrid[1] ** 2 == 0,
            1,
            (2 * np.pi * 1j * (self.qgrid[0] ** 2 + self.qgrid[1] ** 2)),
        )
