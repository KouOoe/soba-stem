...
class SMatrixFwdCalc:
    ...
    def __init__(self, voltage, aperture_mrad, gpu_use=True):
        ...
        self.device_kind = torch.device(
            "cuda" if (torch.cuda.is_available() & gpu_use) else "cpu"
        )
    ...
    def make_grid(self, ry, rx, ny, nx):
        ...
        self.rgrid = torch.meshgrid(
            torch.arange(0, ry, self.dry, device=self.device_kind),
            torch.arange(0, rx, self.drx, device=self.device_kind),
            indexing="ij",
        )
        self.qgrid = torch.meshgrid(
            torch.fft.fftfreq(self.nry, self.dry, device=self.device_kind),
            torch.fft.fftfreq(self.nrx, self.drx, device=self.device_kind),
            indexing="ij",
        )

    def import_beams(self, beam_coordinates, beam_indices):
        self.n_beam = len(beam_indices[0])
        self.beam_coordinates = torch.tensor(
            np.array(beam_coordinates), device=self.device_kind
        )
        self.beam_indices = torch.tensor(
            np.array(beam_indices), device=self.device_kind
        )
    ...
    def make_Amatrix_from_pot_vector(self, Ug_pot):
        ...
        A_matrix = (
            Ug_els[diff_indices_x, diff_indices_y]
            + 1j * Ug_abs[diff_indices_x, diff_indices_y]
        )
    ...
    def expm_pade_scaled_torch_optimized(self, A, m):
        ...
        n = A.size(0)
        c = torch.tensor(
            np.array([1, 1 / 2, 1 / 6, 1 / 24, 1 / 120, 1 / 720, 1 / 5040, 1 / 40320]),
            dtype=dtype,
            device=device,
        )
        ...
        V = sum(
            ci * (-pi if i % 2 else pi) for i, (ci, pi) in enumerate(zip(c, powers))
        )
    ...
    def phasefactor_probe_vector(self, ry, rx, k):
        # Extract ky and kx from k.
        ky = k[0, :]
        kx = k[1, :]

        # Generate a 2D grid from ry and rx.
        ry_grid, rx_grid = torch.meshgrid(ry, rx, indexing="ij")

        # Compute the phase factor with ky and kx at each grid point.
        # Use unsqueeze for broadcasting.
        phase_factor = torch.exp(
            -pi_tensor * 2j * (ry_grid.unsqueeze(-1) * ky + rx_grid.unsqueeze(-1) * kx)
        )
    ...
    def phasefactor_probe_vector_as_stack(self, ry, rx, k):
        ...
        # Generate a stacked grid from ry and rx.
    ...
    def D_torch(
        ...
        kx_rot = kx * torch.cos(rot_offset + rot_epsilon) + ky * torch.sin(
            rot_offset + rot_epsilon
        )
        ky_rot = -kx * torch.sin(rot_offset + rot_epsilon) + ky * torch.cos(
            rot_offset + rot_epsilon
        )

        mask_radius = torch.logical_and(
            ky**2 + kx**2 < outer**2, ky**2 + kx**2 >= inner**2
        )
        mask_azimuth = torch.logical_and(
            torch.atan2(ky_rot, kx_rot) >= theta1, torch.atan2(ky_rot, kx_rot) < theta2
        )
    ...
    def A_torch_soft(self, ky, kx):
        soft_aperture = torch.where(
            ky**2 + kx**2 > 0,
            (self.k0 * torch.sqrt(ky**2 + kx**2) - (ky**2 + kx**2))
            / torch.sqrt(((ky * self.dqy) ** 2 + (kx * self.dqx) ** 2)),
            torch.ones(ky.size(), device=self.device_kind, dtype=torch.float64),
        )
        soft_aperture = torch.where(
            soft_aperture + 0.5 > 0,
            soft_aperture + 0.5,
            torch.zeros(ky.size(), device=self.device_kind, dtype=torch.float64),
        )
        soft_aperture = torch.where(
            soft_aperture > 1,
            torch.ones(ky.size(), device=self.device_kind, dtype=torch.float64),
    ...
    def chi_torch(
        ...
        ky,
        kx,
        defocus=torch.tensor(0),
        tilt_rad=torch.tensor([0, 0]),
        tilt_z=torch.tensor(0),
        coma=torch.tensor([0, 0]),
        twofold_stig=torch.tensor([0, 0]),
        threefold_stig=torch.tensor([0, 0]),
    ):
        ...
        chi = (
            pi_tensor * defocus * (self.wavelength * (ky**2 + kx**2))
            - 2
            * pi_tensor
            * tilt_z
            * (ky * torch.tan(tilt_rad[0]) + kx * torch.tan(tilt_rad[1]))
            + 2
            / 3
            * pi_tensor
            * self.wavelength**2
            * (ky**2 + kx**2)
            * (coma[0] * ky + coma[1] * kx)
            + twofold_stig[0] * pi_tensor * self.wavelength * (kx**2 - ky**2)
            + 2 * twofold_stig[1] * pi_tensor * self.wavelength * ky * kx
            + 2
            / 3
            * pi_tensor
            * self.wavelength**2
            * (
                threefold_stig[0] * ky * (ky**2 - 3 * kx**2)
                + threefold_stig[1] * kx * (kx**2 - 3 * ky**2)
            )
        )
        return chi
    ...
    def T_torch(
        ...
        lens_transfer = self.A_torch_soft(ky, kx) * torch.exp(
            -1j
            * self.chi_torch(
                ky, kx, defocus, tilt_rad, tilt_z, coma, twofold_stig, threefold_stig
            )
        )
        # self.A_torch(ky,kx) * torch.exp(-1j*self.chi_torch(ky,kx,defocus,tilt_rad))
        return lens_transfer

    def calc_CBED_from_Smatrix_vector_from_imported_scan_pos(
        self,
        smatrix,
        chunk_size=16,
        defocus=0,
        ...
        self.ky_cbed = torch.fft.ifftshift(
            torch.fft.fftshift(self.qgrid[0])[ystart:yend, xstart:xend]
        )
        self.kx_cbed = torch.fft.ifftshift(
            torch.fft.fftshift(self.qgrid[1])[ystart:yend, xstart:xend]
        )
        ...
        for i in range(chunk_ny):
            for j in range(chunk_nx):
                ...
                phase_factor = self.T_torch(
                    self.beam_coordinates[0],
                    self.beam_coordinates[1],
                    defocus,
                    tilt_rad,
                    tilt_z,
                    coma,
                    twofold_stig,
                    threefold_stig,
                ) * self.phasefactor_probe_vector_as_stack(
                    rpy_chunk, rpx_chunk, self.beam_coordinates
                )
                ...
                amplitude = torch.einsum(
                    "...h,gh->...g", phase_factor, smatrix
                )  # torch.matmul(phase_factor, smatrix)
                intensity = (
                    torch.abs(amplitude) ** 2
                ).float()  # shape: (ny_chunk, nx_chunk, n_beam)

                # Preallocate a temporary CBED chunk and assign the computed intensities
                cbed_chunk = torch.zeros(
                    ny_chunk,
                    nx_chunk,
                    ny_cbed,
                    nx_cbed,
                ...
                cbed[start_y:end_y, start_x:end_x, :, :] = cbed_chunk / torch.sum(
                    torch.abs(
                        self.T_torch(
                            self.beam_coordinates[0], self.beam_coordinates[1], defocus
                        )
                    )
                    ** 2
                )
    ...
    def source_blur_cbed_GandL(
        self, cbed, n_scan, source_fwhm_G, source_fwhm_L, ratio_GtoL=1, axis=(0, 1)
    ):

        ny_scan = n_scan[0]
        nx_scan = n_scan[1]
        rpy_ary = torch.fft.fftfreq(
            ny_scan, self.ry / ny_scan, device=self.device_kind
        )  # torch.arange(0, self.ry, dr_scan)
        rpx_ary = torch.fft.fftfreq(
            nx_scan, self.rx / nx_scan, device=self.device_kind
        )  # torch.arange(0, self.rx, dr_scan)
        sgrid = torch.meshgrid(rpy_ary, rpx_ary, indexing="ij")

        source_sigma_G = source_fwhm_G / (
            2 * torch.sqrt(2 * torch.log(torch.tensor(2, device=self.device_kind)))
        ).to(self.device_kind)
        source_sigma_L = source_fwhm_L / (
            2 * torch.sqrt(2 * torch.log(torch.tensor(2, device=self.device_kind)))
        ).to(self.device_kind)

        gauss_filter = torch.exp(
            -((sgrid[0] ** 2 + sgrid[1] ** 2) * pi_tensor**2 * 2 * source_sigma_G**2)
        ).to(self.device_kind)
        lorentz_filter = torch.exp(
            -source_sigma_L * pi_tensor * torch.sqrt(sgrid[0] ** 2 + sgrid[1] ** 2)
        )

        blur_filter = gauss_filter * ratio_GtoL + lorentz_filter * (1 - ratio_GtoL)

        cbed_blur = torch.fft.ifft2(
            torch.fft.fft2(cbed, dim=axis) * blur_filter.unsqueeze(-1).unsqueeze(-1),
            dim=axis,
        ).real
