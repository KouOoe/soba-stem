...
class SMatrixPreProcess:
    ...
    def make_grid(self, ry, rx, ny, nx):
        ...
        self.rgrid = np.meshgrid(
            np.arange(0, ry, self.dry), np.arange(0, rx, self.drx), indexing="ij"
        )
        self.qgrid = np.meshgrid(
            np.fft.fftfreq(self.nry, self.dry),
            np.fft.fftfreq(self.nrx, self.drx),
            indexing="ij",
        )

    def set_beams_from_image(
        self,
        image_fft,
        amp_cutoff=0,
        show_beams=True,
        cutoff_k_frac=1,
        cutoff_k_inner_frac=0,
        show_nbeams=True,
    ):
        ...
        pot_fft_crop = np.fft.fftshift(image_fft)[
            self.nry // 2 - ny_crop : self.nry // 2 + ny_crop,
            self.nrx // 2 - nx_crop : self.nrx // 2 + nx_crop,
        ]
        cutoff_k = (
            np.sqrt(self.qgrid[0] ** 2 + self.qgrid[1] ** 2) < self.k0 * cutoff_k_frac
        ) & (
            np.sqrt(self.qgrid[0] ** 2 + self.qgrid[1] ** 2)
            >= self.k0 * cutoff_k_inner_frac
        )
        ...
        sorted_coordinates, sorted_indices = self.sorted_coordinates_within_cutoff(
            image_fft, cutoff_k, amp_cutoff=amp_cutoff
        )

        if cutoff_k_inner_frac == 0:
            self.beam_coordinates = np.concatenate(
                (-np.flip(sorted_coordinates[0]), np.array([0]), sorted_coordinates[0])
            ), np.concatenate(
                (-np.flip(sorted_coordinates[1]), np.array([0]), sorted_coordinates[1])
            )
            self.beam_indices = (
                np.array(
                    [
                        (self.beam_coordinates[0] / self.dqy),
                        (self.beam_coordinates[1] / self.dqx),
                    ]
                )
                .round()
                .astype(int)
            ...
            self.beam_coordinates = np.concatenate(
                (-np.flip(sorted_coordinates[0]), sorted_coordinates[0])
            ), np.concatenate((-np.flip(sorted_coordinates[1]), sorted_coordinates[1]))
            self.beam_indices = (
                np.array(
                    [
                        (self.beam_coordinates[0] / self.dqy),
        ...
        if show_beams:
            ...
            plt.scatter(
                ...
                c=self.beam_coordinates[0] ** 2 + self.beam_coordinates[1] ** 2,
            )
            ...
            plt.scatter(
                self.beam_indices[1],
                self.beam_indices[0],
                c=self.beam_indices[0] ** 2 + self.beam_indices[1] ** 2,
            )
    ...
    def sorted_coordinates_within_cutoff(self, image_fft, cutoff_k, amp_cutoff=0):
        ...
        y_indices, x_indices = np.where(
            (cutoff_k)
            & (np.arctan2(self.qgrid[0], self.qgrid[1]) > 0)
            & (np.abs(image_fft) >= amp_cutoff)
        )
        ...
        filtered_ky = self.qgrid[0][
            (cutoff_k)
            & (np.arctan2(self.qgrid[0], self.qgrid[1]) > 0)
            & (np.abs(image_fft) >= amp_cutoff)
        ]
        filtered_kx = self.qgrid[1][
            (cutoff_k)
            & (np.arctan2(self.qgrid[0], self.qgrid[1]) > 0)
            & (np.abs(image_fft) >= amp_cutoff)
        ]
        ...
        unique_distances, unique_starts, unique_counts = np.unique(
            filtered_distances[sort_indices], return_index=True, return_counts=True
        )
    ...
    def Ug_circle_plot(
        ...
        Ug_1,
        Ug_2,
        display_factor_kmax=2.5,
        circle_size_factor=150,
        size_cutoff=0.1,
    ):
        ...
        plt.scatter(
            ...
            self.qgrid[1],
            s=np.where(
                np.abs(Ug_1) * size_factor > size_cutoff, np.abs(Ug_1) * size_factor, 0
            ),
            c=np.mod(np.angle(Ug_1) - np.angle(Ug_2) + np.pi, np.pi * 2) - np.pi,
            cmap="hsv",
            vmin=-np.pi,
            vmax=np.pi,
            alpha=0.5,
        ...
        plt.scatter(
            ...
            self.qgrid[1],
            s=np.where(
                np.abs(Ug_2) * size_factor > size_cutoff, np.abs(Ug_2) * size_factor, 0
            ),
            alpha=1,
            linewidths=1,
            edgecolors="black",
        ...
        plt.plot(
            self.aperture * 1e-3 / self.wavelength * np.cos(theta),
            self.aperture * 1e-3 / self.wavelength * np.sin(theta),
            linestyle="--",
            color="grey",
            linewidth=1.44,
        )
