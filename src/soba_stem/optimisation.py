...
def run_optimiser(
    ...
    nky_crop_losscalc = int(
        config["kmax_crop_loss_calc"]
        * smatrix_preprocess.k0
        / data_loader_params["sampling_diff"][0]
        * 2
    )
    nkx_crop_losscalc = int(
        config["kmax_crop_loss_calc"]
        * smatrix_preprocess.k0
        / data_loader_params["sampling_diff"][1]
        * 2
    )
    com_measured_data = ndimage.center_of_mass(
        torch.sum(measured_data, dim=(0, 1)).cpu().detach().numpy()
    )
    ...
    end_y = (
        start_y + nky_crop_losscalc
    )  # round(com_measured_data[0].to('cpu').detach().numpy()+nky_crop_losscalc/2) #
    end_x = (
        start_x + nkx_crop_losscalc
    )  # round(com_measured_data[1].to('cpu').detach().numpy()+nkx_crop_losscalc/2) #
    ...
    thickness = initial_params[
        "thickness"
    ]  # torch.tensor, requires_grad set as in config
    ...
    filter_els = torch.ones_like(
        torch.tensor(Ug_from_OBF), dtype=torch.complex64, device=device
    )  # np.ones(np.shape(Ug_from_OBF), dtype=np.complex64)
    filter_abs = torch.ones_like(
        torch.tensor(Ug_from_OBF), dtype=torch.complex64, device=device
    )  # np.ones(np.shape(Ug_from_OBF), dtype=np.complex64)

    # diff space sampling ratio (estimated to measured data) and rotation offset
    pixel_ratio_diff_est_to_meas_y = (
        smatrix_fwd_calc.dqy / data_loader_params["sampling_diff"][0]
    )
    pixel_ratio_diff_est_to_meas_x = (
        smatrix_fwd_calc.dqx / data_loader_params["sampling_diff"][1]
    )
    ...
    for i_kcutoff, (lr, n_iter, kmax_cutoff_val) in enumerate(
        zip(config["lr_ary"], config["n_iter_ary"], config["kmax_optim_ary"])
    ):
        ...
        kmin_cutoff = np.where(
            (i_kcutoff == 0) or (config["kmin_cutoff_adjust"] == False),
            0,
            config["kmax_optim_ary"][i_kcutoff - 1],
        )
        # Optionally also update kmin_cutoff as needed:
        kmax_cutoff_beam = (
            1
            - np.max([smatrix_preprocess.dqy, smatrix_preprocess.dqx])
            / smatrix_preprocess.k0
        ) * kmax_cutoff
        kmin_cutoff_beam = (
            1
        ...
        Wg_from_OBF = Ug_from_OBF[
            smatrix_preprocess.beam_indices[0], smatrix_preprocess.beam_indices[1]
        ]
        ...
        Wg_els = (
            filter_els[
                smatrix_preprocess.beam_indices[0, : smatrix_preprocess.n_beam // 2],
        ...
        if config["optim_potential"]:
            ...
            param_groups.append(
                {"params": [Wg_abs], "lr": current_lr}
            )  # * config['abs_pot_factor']})
            ...
            param_groups.append(
                {
                    "params": [source_fwhm],
                    "lr": current_lr
                    * config["lr_factor_sourcesize"]
                    * source_fwhm.item(),
                }
            )
        if config["optim_tilt"]:
            param_groups.append(
                {"params": [tilt_rad], "lr": current_lr * config["lr_factor_tilt"]}
            )
        if config["optim_coma"]:
            param_groups.append(
                {"params": [coma], "lr": current_lr * config["lr_factor_coma"]}
            )
        ...
        if config["optim_probe_position"]:
            param_groups.append(
                {
                    "params": [probe_position],
                    "lr": current_lr * config["lr_factor_probe_position"],
                }
            )

        # tensor for the initial potential
        obf_initial = torch.tensor(
            Wg_from_OBF, device=device, requires_grad=False, dtype=torch.complex64
        )
        ...
        for iter_idx in range(n_iter):
            ...
            pot_optim = torch.stack(
                [
                    torch.tensor(Ug_from_OBF, device=device, dtype=torch.complex64)
                    * filter_els,
                    torch.tensor(Ug_from_OBF, device=device, dtype=torch.complex64)
                    * config["abs_pot_factor"]
                    * filter_abs,
            ...
            pot_optim[
                ...
                obf_initial[: smatrix_preprocess.n_beam // 2]
                * config["abs_pot_factor"]
                * Wg_abs
            )
            ...
            if kmin_cutoff == 0:
                pot_optim[
                    0,
                    smatrix_preprocess.beam_indices[
                        0,
                        smatrix_preprocess.n_beam // 2 + 1 : smatrix_preprocess.n_beam,
                    ],
                    smatrix_preprocess.beam_indices[
                        1,
                        smatrix_preprocess.n_beam // 2 + 1 : smatrix_preprocess.n_beam,
                    ...
                    (Wg_els).real + 1j * (-Wg_els).imag, dims=(0,)
                )
                ...
                pot_optim[
                    ...
                    * torch.flip((Wg_abs).real + 1j * (-Wg_abs).imag, dims=(0,))
                    * config["abs_pot_factor"]
                )
            else:
                pot_optim[
                    0,
                    smatrix_preprocess.beam_indices[
                        0, smatrix_preprocess.n_beam // 2 : smatrix_preprocess.n_beam
                    ],
                    smatrix_preprocess.beam_indices[
                        1, smatrix_preprocess.n_beam // 2 : smatrix_preprocess.n_beam
                    ],
                ] = obf_initial[
                    smatrix_preprocess.n_beam // 2 : smatrix_preprocess.n_beam
                ] * torch.flip(
                    (Wg_els).real + 1j * (-Wg_els).imag, dims=(0,)
                )
                pot_optim[
            ...
            Amatrix_tmp = smatrix_fwd_calc.make_Amatrix_from_pot_vector(
                [pot_optim[0], pot_optim[1]]
            )
            ...
            estimated_data = torch.zeros(
                measured_data.shape, device=device, dtype=torch.float32
            )
            estimated_data = (
                smatrix_fwd_calc.calc_CBED_from_Smatrix_vector_from_imported_scan_pos(
                    smatrix=Smatrix_tmp,
            ...
            if config["lowpass_measured_data"]:
                estimated_data = torch.fft.ifft2(
                    torch.fft.fft2(estimated_data, dim=(-2, -1))
                    * (
                        (
                            smatrix_fwd_calc.qgrid[0] ** 2
                            + smatrix_fwd_calc.qgrid[1] ** 2
                        )
                        < (smatrix_fwd_calc.k0 * config["kmax_optim"]) ** 2
                    )
                ).real

            if (
                pixel_ratio_diff_est_to_meas_x.item() != 1
                or pixel_ratio_diff_est_to_meas_y.item() != 1
                or rotation_offset_diff != 0
            ):
                # resize and rotate the estimated data to match the measured data
                nky_resize_estimated = round(
                    estimated_data.size()[2] * pixel_ratio_diff_est_to_meas_y.item()
                )
                nkx_resize_estimated = round(
                    estimated_data.size()[3] * pixel_ratio_diff_est_to_meas_x.item()
                )

                # if config["pixel_ratio_diff_est_to_meas"]==[1,1]:
                estimated_data = cbed_resize_rotation(
                    estimated_data,
            ...
            """
            com_estimated_data = ndimage.center_of_mass(
                torch.sum(estimated_data, dim=(0, 1)).cpu().detach().numpy()
            )
            ...
            if "loss_function" in config:
                if config["loss_function"] == "L2":
                    loss = torch.norm(estimated_data - measured_data) ** 2 + config[
                        "regularize_factor"
                    ] * torch.sum(torch.abs(pot_optim))
                elif config["loss_function"] == "L2_normalised":
                    loss = torch.norm(
                        estimated_data
                        / torch.sum(estimated_data, dim=(2, 3), keepdims=True)
                        - measured_data
                        / torch.sum(measured_data, dim=(2, 3), keepdims=True)
                    ) ** 2 + config["regularize_factor"] * torch.sum(
                        torch.abs(pot_optim)
                    )
                elif config["loss_function"] == "L1":
                    loss = torch.norm(estimated_data - measured_data, p=1) + config[
                        "regularize_factor"
                    ] * torch.sum(torch.abs(pot_optim))
                elif config["loss_function"] == "pearson":
                    loss = -(
                        (estimated_data - estimated_data.mean())
                        / estimated_data.std()
                        * (measured_data - measured_data.mean())
                        / measured_data.std()
                    ).mean()
                    ...
                    loss = torch.sum((estimated_data - measured_data) ** 2) + config[
                        "regularize_factor"
                    ] * torch.sum(torch.abs(pot_optim))
                    ...
                    loss = torch.sum(
                        (
                            estimated_data * config["dose_measured"]
                            - measured_data * config["dose_measured"]
                        )
                        ** 2
                        * 1
                        / (estimated_data * config["dose_measured"])
                ...
                loss = torch.norm(
                    ...
                    - measured_data
                    / torch.sum(measured_data, dim=(2, 3), keepdims=True)
                ) ** 2 + config["regularize_factor"] * torch.sum(torch.abs(pot_optim))
            ...
            print(
                f"Iteration {iter_idx + 1}/{n_iter} of kcutoff group {i_kcutoff+1}/{len(config['kmax_optim_ary'])}: Loss = {loss.item():.4f} - {progress_percent:.1f}% complete",
                end="\r",
                flush=True,
            )
        ...
        if kmin_cutoff == 0:
            potential_els[
                smatrix_preprocess.beam_indices[0, 0 : smatrix_preprocess.n_beam // 2],
            ...
            potential_els[
                smatrix_preprocess.beam_indices[
                    0, smatrix_preprocess.n_beam // 2 + 1 : smatrix_preprocess.n_beam
                ],
                smatrix_preprocess.beam_indices[
                    1, smatrix_preprocess.n_beam // 2 + 1 : smatrix_preprocess.n_beam
                ],
            ] = obf_initial[
                smatrix_preprocess.n_beam // 2 + 1 : smatrix_preprocess.n_beam
            ] * torch.flip(
...
def cbed_resize_rotation(cbed, angle_rad, size, interpolate_mode="bicubic"):
    ...
    rotated_image = F.grid_sample(
        cbed, grid, mode=interpolate_mode, padding_mode="zeros", align_corners=False
    )
